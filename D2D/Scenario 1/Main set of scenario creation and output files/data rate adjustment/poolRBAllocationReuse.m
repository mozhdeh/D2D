...
%order not to mix things with Cell2.
 ...
 %First loading the text file "DisjointIndependentSubsets.txt" from "myshellmodeDR3DExtension"
...
if newLCRBCell1==LCRBdefaultCell1;%in case of no rate adaptation
   ...
   for k=1:20;%note that here instead of "numberOfBG", we put "20", in order to run the file in batch mode without need for commenting extra lines
       ...
       thesize=size(y);
       if thesize(1,1)~=100; % 100 here is "50(empty initialzied y)+50 (kronned)=100". this if checks if there is any uncovered single D2D RBs to be added to "y" before concatenation with "z"
           
          %we first need to know how many uncovered are there at each side
          %of spectrum
         nrofuncovered=(100-thesize(1,1))/2;%actually it is calculated also above in line 124
         %here is a mini for loop to add zero rows for the uncovered RBs at
         %top side
          b=zeros(1,8);
         for num=1:nrofuncovered;
             
             row_no=newLCRBCell1*(xSRB)+50+num; %insert for num_th uncovered
         ...
         thissize=size(y);
         for num=1:nrofuncovered;             
             row_no=thissize(1,1)-num-(newLCRBCell1*(xSRB))+1; %insert for num_th uncovered
             y(1:row_no-1,:) = y(1:row_no-1,:);
             tp =y(row_no:end,:);
             y(row_no,:)=b;
             y(row_no+1:end+1,:) =tp;           
...
% % %           y(row_no+1:end+1,:) =tp;
   ...
   %we need to remove the last all zero page of "z" that was
       %initialized 
       reuseRBCell1new=reuseRBCell1new(:,:,1:20);%note that here instead of "numberOfBG", we put "20", in order to run the file in batch mode without need for commenting extra lines
       reuseRBCell1newreverse=zeros(50,8,20);%note that here instead of "numberOfBG", we put "20", in order to run the file in batch mode without need for commenting extra lines
       ...
       num=0;
       for page=1:20;%note that here instead of "numberOfBG", we put "20", in order to run the file in batch mode without need for commenting extra lines
           reuseRBCell1newreverse(:,:,page)=reuseRBCell1new(:,:,20-num);
           num=num+1;
       end;
