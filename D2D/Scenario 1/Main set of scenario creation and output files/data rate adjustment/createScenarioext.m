...
%The following for loop is with rate adjustment
  if LI<=1;
      poolRBAllocationScheduledext;     
  elseif (1<LI && LI<=LIth);%no need for rate adaptation yet
      ...
      %MySubsets = dlmread('DisjointIndependentSubsets.txt');%commented due
      %to batching
      %save('MySubsets.mat','MySubsets')%commented due
      %to batching
      load('MySubsets.mat','MySubsets')%is loaded from the "myshellmodeDR3DExtension"
      mysetsize=size(MySubsets);%save here to be used later for computing "se" 
      mysetsizenew=mysetsize(1,1);%save here to be used later for computing "se": line 346
      ...
      %here we first check if "|result|<=D2DresourcesSRBDefaultCell1", if
      %so, no rate adjustment is done and if not, we do rate adjustment
      %first we import the "DisjointIndependentSubsets.txt" generated in
      %mathematica
     %MySubsets = dlmread('DisjointIndependentSubsets.txt');%commented due
      %to batching
      %save('MySubsets.mat','MySubsets')%commented due
      ...
      mysetsizenew=mysetsize(1,1);
      
      if mysetsizenew <= D2DresourcesSRBDefaultCell1;
         %consider the same default setting as the new ones; we do this rename
         %to avoid errors while loading variables in "poolRBAllocationReuse".
         newD2DresourcesSRBCell1=D2DresourcesSRBDefaultCell1;
         xSRB=newD2DresourcesSRBCell1/2; 
         ...
         %error while loading them with new names in "poolRBAllocationReuse".
         save('myvariables.mat','xSRB','newLCRBCell1','LCRBdefaultCell1','nrResourcesTotalCell1','LI')
         %we save separately "newLCRBCell1" for the rate adjustment mode,
         %to be loaded and saved in a text file in the corresponding batch
         %file. this is needed to be shown later in the graphof DR
         %(example) that what is the data rate at each part of the curve
         save('newLCRBCell1.mat','newLCRBCell1');
         dlmwrite('LI.txt',LI)
         poolRBAllocationReuse; 
      else
...
if LI<=1; %scheduled mode
   templines = numberOfBG;
else %reuse mode
   templines=mysetsizenew;
end;
lines=templines;
...
% % lines=templines;
% % save('lines.mat','lines');
% % se=(numberOfBG*graph1sched)/templines;
% % save('se.mat','se')
...
% % for i=1:numberOfBG;
% %     temp=sum(bigReceptionMatrixSched2(i,:)==0)/(numberOfPackets*numberOfReceivers);
% %     finaltemp1=horzcat(finaltemp1,temp);
% % end;
